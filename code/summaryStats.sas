...
ods html body ="&dirpath&dsn._env_specific_analysis_output.html";
...
proc means data=&dsn._blup nway noprint ;
	...
	id trial;
run;
proc print data=&dsn._ave_pev;run;
...
run;
proc sort data=&dsn._trial_summary;
	by trait env;
run;
