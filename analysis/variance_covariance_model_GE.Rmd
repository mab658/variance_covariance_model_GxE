...
DIAG <- asreml(fixed = fyld ~ env/set + prop_hav,
        random =~at(env):(set:rep) + diag(env):vm(gen,Hinv) + , # additive nested genetic effect within envs
        diag(env):ide(gen), # non-additive nested genetic  effect within envs
...
#  extract additive variance matrix
sigma2a <- diag(DIAGvar[grep("env.*vm", rownames(DIAGvar)),1])

# non-additive variance matrix
sigma2i <- diag(DIAGvar[grep("env.*ide", rownames(DIAGvar)),1])

# total genetic variance matrix
varCovMt  <- round(sigma2a* mean(diag(Hmat)) + sigma2i,2)
...
CS <- asreml(fixed=fyld~env/set + prop_hav,
             random=~at(env):(set:rep) +
               vm(gen,Hinv) + # additive main genetic effect
               idv(env):vm(gen,Hinv) + # additive gxe interaction effect

               ide(gen) +  # non-additive main genetic effect across environment
               idv(env):ide(gen), # non-additive gxe interaction effect
...
#  extract main additive and gxe variance matrices
sigma2a <- CSvar[grep("^vm", rownames(CSvar)),1]  # additive genetic var across envs
sigma2a.ge <- CSvar[grep("env.*vm", rownames(CSvar)),1] #  genetic var x environments


# extract main non-additive and gxe variance matrices
sigma2i <- CSvar[grep("^ide", rownames(CSvar)),1]
sigma2i.ge <- CSvar[grep("env.*ide", rownames(CSvar)),1]


# Compute total main genetic  and gxe effect variance matrices
sigma2g <- sigma2a* mean(diag(Hmat)) + sigma2i
sigma2ge <- sigma2a.ge * mean(diag(Hmat)) + sigma2i.ge

cat("\n Correlation variance covariance matrix  from CS  model","\n")

varCovMt <- matrix(sigma2g, ncol = ne, nrow = ne) + diag(sigma2ge, nrow = ne)
...
corrVarCovMt <- round(varCovMt,2)

print(corrVarCovMt)
write.csv(corrVarCovMt, file = "output/CS_corrVarCov.csv")
```
...
CSH <- asreml(fixed=fyld~env/set + prop_hav,
                random=~at(env):(set:rep) + 
                vm(gen,Hinv) + # additive main effect
                idv(env):vm(gen,Hinv) + # additive gxe interaction effect
                
                ide(gen) +  # non-additive main effect across env
                idv(env):ide(gen) +  # non-additive gxe interaction effect
                corh(env):vm(gen,Hinv), # uniform genetic correlation within environments
                residual=~dsum(~units|env),
                na.action=na.method(y='include',x='include'),
                workspace=250e6,maxit=50, data=pheno)
...
# extract main additive genetic and gxe  variance matrices
sigma2a <- CSHvar[grep("^vm", rownames(CSHvar)),1] 
sigma2a.ge <- CSHvar[grep("env:.*_.*", rownames(CSHvar)),1] 

# Extract main non-additive and gxe variance matrices
sigma2i  <- CSHvar[grep("^ide", rownames(CSHvar)),1]
sigma2i.ge <- CSHvar[grep("env.*ide", rownames(CSHvar)),1]

# Compute total main genetic  and gxe effect variance matrices
sigma2g <- sigma2a * mean(diag(Hmat)) + sigma2i
sigma2ge <- sigma2a.ge * mean(diag(Hmat)) + sigma2i.ge

# extract genetic correlation value b/w pairs of environment
gencorr  <- CSHvar[grep("env.*cor", rownames(CSHvar)),1] # additive genetic x environment variance

cat("\n Correlation variance covariance matrix from CSH model","\n")

varCovMt <- diag(sigma2ge) + diag(sigma2g,nrow=ne,ncol=ne)
rownames(varCovMt) <- unique(levels(pheno$env))
colnames(varCovMt) <- unique(levels(pheno$env))

varCovMt[lower.tri(varCovMt)] <-  gencorr
# compute genetic covariances for above main diagonal elements
for (r in 1:nrow(varCovMt)){
  for (c in 2:ncol(varCovMt)){

    if((varCovMt[r,c]==0) && (r != c)){
      varCovMt[r,c] <- gencorr*sqrt(varCovMt[r,r])*sqrt(varCovMt[c,c])
    }
...
write.csv(corrVarCovMt, file ="output/CSH_corrVarCov.csv")
```
...
# additive genetic variance  variance matrix
sigma2a <- lambda %*% t(lambda) +  diag(psi)

rownames(sigma2a) <- unique(levels(pheno$env))
colnames(sigma2a) <- unique(levels(pheno$env))

# non-additive variance matrix
sigma2i <- diag(FA1$fa.vars[grep("env.*ide", rownames(FA1$fa.vars)),1])

# total genetic variance covariance matrix
varCovMt  <- sigma2a * mean(diag(Hmat)) + sigma2i

corrMt <- round(cov2cor(varCovMt),2)
varCovMt[lower.tri(varCovMt)] <-  corrMt[lower.tri(corrMt)]

print(varCovMt)
write.csv(varCovMt, file = "FA1_corrVarCov.csv")

gen.var  <- as.vector(diag(varCovMt)) # genetic variance
...
# Compute additive genetic variance  matrix
sigma2g <- L.star %*% t(L.star) + diag(psi)

# non-additive variance matrix
sigma2i <- diag(FA2$fa.vars[grep("env.*ide", rownames(FA2$fa.vars)),1])

# total genetic variance covariance  matrix
varCovMt  <- sigma2a * mean(diag(Hmat)) + sigma2i
corrMt <- cov2cor(varCovMt)
varCovMt[lower.tri(varCovMt)] <-  corrMt[lower.tri(corrMt)]

corrVarCovMt <- round(varCovMt,2)
print(corrVarCovMt)
write.csv(corrVarCovMt, file = "output/FA2_corrVarCov.csv")

# Compute the percentage of genetic variance attributed to factor loadings
gen.var <- as.vector(diag(varCovMt))
...
FA2.totVar  <- mean(totGenVar) # total genetic variance captured by FA2  model


# Percentage of variance explained by the latent factor
...
# Compute additive genetic variance covariance  matrix
sigma2a <- L.star %*% t(L.star) + diag(psi)


# non-additive variance matrix
sigma2i <- diag(FA3$fa.vars[grep("env.*ide", rownames(FA3$fa.vars)),1])
...
corrVarCovMt <- round(varCovMt,2)
print(corrVarCovMt)
write.csv(varCovMt, file = "output/FA3_corrVarCov.csv")
write.csv(corrMt, file = "output/FA3_genetic_corr.csv")
...
FA3.totVar  <- mean(totGenVar) # total genetic variance captured by FA3 model
...
##  Singular value decomposition of loadings from FA4 model to have  unique solution
svd.FA4 <- svd(lambda)
...
# non-additive variance matrix
sigma2i <- diag(FA4$fa.vars[grep("env.*ide", rownames(FA4$fa.vars)),1])
...
write.csv(corrvarCovMt, file = "output/FA4_corrVarCov.csv")
write.csv(corrMt, file = "output/FA4_genetic_corr.csv")
...
# Likelihood Ratio Test between FA2 vs FA3 and FA3 vs FA4 models
...
# Interactive classes (iClasses)
This chunk identifies a cluster of environments showing low cross-over genotype-by-environment interaction

```{r}
# extract out the first three loadings for iCLASS
fa3Load <- FA3.var[,c("env","fa1","fa2","fa3")]

fa3Load$fac1 <- 'p'
fa3Load$fac2 <- factor(fa3Load$fa2 > 0,labels = c('n','p'))
fa3Load$fac3 <- factor(fa3Load$fa3 > 0,labels = c('n','p'))

# create iCclass factor by pasting these levels
fa3Load$iClass <- factor(paste(fa3Load$fac1,fa3Load$fac2,fa3Load$fac3,sep=''))
fa3Load <- fa3Load[,c("env","fa1","fa2","fa3","iClass")]

 table(fa3Load$iClass) # number of environment per iClasses

print(fa3Load)
write.csv(fa3Load, file="output/fa3_iClass.csv", row.names=FALSE)
```
...
This section showed how genotype stability may be best viewed using latent regression plots which revealed genotypic responses to each rotated factor loading. This involves calculating the common variety or genotypic effect from linear regression of the form

$ u_{ij}= \lambda_{1j}f_{1i} + \lambda_{2j}f_{2i} + \lambda_{3j}f_{3i} + \psi$
...
# First two (OP), the next two (stability), last two genotypes(High industrial starch content)

genList <- c("TMS13F1376P0018","IITA-TMS-IBA000070", 
             "TMS14F1306P0020","TMS13F1365P0029",
...
This script reads in the factor loadings from FA3 model as responses and weather data at developmental phase of cassava as predictors. We maximize the covariances between the responses and predictors using partial least squares regression.

```{r}
# Read two-way table of env x gen predicted field value
envLoading <- read.csv(file="data/FA3_loadings.csv", header=T)[,1:4] # read two-way table

# Read two-way table of env x environmental covariates across developmental phases
envCov <- read.csv(file="data/weatherData_QTRs.csv", header=T)[,-c(6,17,28,39)]
 
# merge the data set together
envCovDat <- merge(envLoading, envCov, by="ENV")

# extract env and AgroZone columns 
envAgroZone <- envCovDat[,c("ENV", "AGROZONE")]

#convert column env to row names
envCovDat <- tibble::column_to_rownames(envCovDat, var="ENV")


# Extract and define a data matrix X of predictor variables and normalized or standardized
# each covariate to mean = 0 var=1 from the combined data
envCov <- scale(as.matrix(envCovDat[,5:ncol(envCovDat)]))


# Extract and define a data matrix Y  of responses (env x factor) loading scores
envScore <- scale(as.matrix(envCovDat[,1:3]))
```
...
# fit partial least square regression model

```{r}
# fit Partial least square regression model on multiple responses using
# multiple predictors using data matrix  X and Y
pls.model <- plsr(envScore~envCov,data=envCovDat, validation = "CV")

# Find the number of dimensions with lowest cross validation error
cv = RMSEP(pls.model)
best.dims = which.min(cv$val[estimate = "adjCV", , ]) - 1

# Refit the model with the best number of component
pls.model <- plsr(envScore~envCov,data=envCovDat, ncomp=best.dims, validation = "CV")

# get overview of fitted model and validation result
summary(pls.model)
round(explvar(pls.model),1)


# create a plot to define the number of components
plot(RMSEP(pls.model))

# extract the X and Y scores
Xscore <- scores(pls.model)[,1:2]
Yscore <- Yscores(pls.model)[,1:2]

# extract the X and Y loadings 
Xloading <- loadings(pls.model)[,1:2]
Yloading <- Yloadings(pls.model)[,1:2]
```


# Plot of the first and second X scores

The plot of X-scores is revealing how the testing environments are related to one another based on factor loadings from FA3 model and environmental covariables.


```{r}

# Plot of X scores to provide regression overview of different environments

Xscore <- as.data.frame(Xscore)
Xscore <- tibble::rownames_to_column(Xscore,var="ENV")
colnames(Xscore)[2:3] <- c("Comp1","Comp2")

Xscore <- merge(Xscore,envAgroZone,by="ENV")

XscorePlot <- ggplot(Xscore, aes(x = Comp1, y = Comp2, label=ENV)) + 
  geom_point(aes(shape=AGROZONE), position = "jitter") + 
  geom_vline(xintercept = 0.0, linetype = "dashed", color = "black", size = 0.20) +
  geom_hline(yintercept = 0.0, linetype = "dashed", color = "black", size = 0.20) + 
  theme_bw() +
  geom_text_repel(size=2.0)+
  labs(x="Component 1 (63%, 49%)", y="Component 2 (8%, 41%)") +
  scale_x_continuous(limits=c(-15.5, 15.5))+
  ...
  theme(plot.title = element_text(hjust = 0.5),
      ...
      legend.position = "right"
 ) + guides(shape = guide_legend(title = "Agro-ecological Zone"))

print(XscorePlot)

ggsave("XscorePlot.jpeg",height=4.2, width=6.5, units="in", dpi=300)
```



# plot of X and Y loadings
This reveals the association between the predictors and responses variables

```{r}
Xloading <- as.data.frame(Xloading)
Xloading <- tibble::rownames_to_column(Xloading,var="envCov")
colnames(Xloading)[2:3] <- c("Comp1","Comp2")

Xloading$Factor <- rep("Environmental covariable",times= 40)


Yloading <- as.data.frame(Yloading)
Yloading <- tibble::rownames_to_column(Yloading,var="envCov")
colnames(Yloading)[2:3] <- c("Comp1","Comp2")

Yloading$Factor <- rep("Factor loading",times= 3)

xyDat <- rbind(Xloading,Yloading)

xyDat$Factor <- as.factor(xyDat$Factor)

XYloadingPlot <- ggplot(xyDat, aes(x = Comp1, y = Comp2, label=envCov))  + 
  geom_point(aes(shape=Factor),position = "jitter") + 
  geom_vline(xintercept = 0.0, linetype = "dashed", color = "black", size = 0.20) +
  geom_hline(yintercept = 0.0, linetype = "dashed", color = "black", size = 0.20) +
  theme_bw() +
  ...
  scale_y_continuous(limits=c(-0.44, 0.44))+
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(size = 7, hjust = 0.5),
        axis.text.y = element_text(size = 7, hjust = 0.5, angle = 90),
       axis.title.y = element_text(size = 10, face = "bold"),
...
ggsave("XYloadingPlot.jpeg",height=4.2, width=6.5, units="in", dpi=300)
```
